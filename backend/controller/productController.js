...
async function resolveCategory(parsedCategory) {
  ...
  const name =
    typeof parsedCategory === "string" ? parsedCategory : parsedCategory?.name;
...
export const addProductController = asyncHandler(async (req, res) => {
  ...
  if (!parsedSizes || parsedSizes.length === 0) {
    return res.status(400).json({
      message:
        "Invalid sizes format. Each size must include size and quantity.",
    });
  }

  const discount = Number(req.body.discount) || 0;
  const salePrice = discount > 0 ? numericPrice * (1 - discount / 100) : null;
...
export const getProductByIdController = asyncHandler(async (req, res) => {
  ...
  console.log(product, "productproductproductproductproductproductproduct");
...
export const updateProductController = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { productName, description, price, category, sizes } = req.body;
