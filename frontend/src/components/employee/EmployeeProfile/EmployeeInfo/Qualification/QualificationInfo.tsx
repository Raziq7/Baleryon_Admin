...
function QualificationInfo() {
  ...
  const [editQualificationId, setEditQualificationId] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    if (id) getEmployeeById(Number(id));
  }, [id, getEmployeeById]);
  ...
  return (
    <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      ...
      {selectedEmployee?.qualifications?.length ? (
        ...
        <p className="text-sm text-gray-500">No qualification records found.</p>
      )}
        ...
        <div className="relative w-full overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900">
          {editQualificationId && (
            <EditQualificationForm
              qualificationId={editQualificationId}
